...
When('I submit request to Get Location API', async function (this: ICustomWorld) {
...
When(
  'I submit request to Get Location API with invalid token',
  async function (this: ICustomWorld) {
    bearerAuthHeader.Authorization = `Bearer2 ${sessionVariable.AccessToken}`;
    const context = await request.newContext({ baseURL: 'https://tctsi-api.ltl.xpo.com' });
    const response = await context.get('/locationv3/3.0/locations/CWA', {
      headers: bearerAuthHeader,
    });
